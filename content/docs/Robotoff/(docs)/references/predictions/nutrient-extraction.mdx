...
This model detects the most common nutrition values (proteins, salt, energy-kj,...), either for 100g or per serving using the LayoutLMv3 architecture for Document AI tasks.
</Callout>
...
The `predict` function [^predict_function] takes as input:
- The image (as a Pillow Image)
- The Google Cloud Vision OCR result (as a `OCRResult` object)
</Callout>
...
<Step>
### Perform Inference
The request is sent to Triton server through gRPC to get model predictions.
</Step>
...
<Step>
### Aggregate Entities
The 'O' (OTHER) entity is ignored, and pre-entities with the same entity class are merged together.
</Step>
...
Sometimes the nutrient unit is not present after the value, or the OCR didn't detect the corresponding word. You can either infer a plausible unit given the entity (ex: `g` for proteins, carbohydrates,...) or ignore this entity.
</Callout>

<Callout title="Entity Validation">
We consider an entity invalid if we couldn't extract nutrient value from the `text` field, or if there are more than one entity for a single nutrient. For example, two `proteins_100g` and one `proteins_serving` are considered valid, but two `proteins_100g` are not.
</Callout>
...
[^import_mechanism]: See [this page](/docs/Robotoff/explanations/predictions.mdx) for more details 
