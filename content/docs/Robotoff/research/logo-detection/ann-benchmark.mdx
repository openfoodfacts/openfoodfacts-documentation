...
<Cards>
  <Card title="Faiss" href="https://github.com/facebookresearch/faiss" description="Facebook AI Similarity Search library" />
  <Card title="Redis" href="https://redis.io/docs/stack/search/reference/vectors/" description="Redis vector search capabilities" />
  <Card title="Elasticsearch" href="https://www.elastic.co/fr/blog/introducing-approximate-nearest-neighbor-search-in-elasticsearch-8-0" description="Elasticsearch ANN search implementation" />
</Cards>
...
<Callout type="note">
  The dimension of each vector used in the benchmark is 768, the size of embeddings CLIP outputs before processing through the projection layer. However, in production, as this last layer of the CLIP model is used, the dimension of the embeddings that are indexed is actually 512. The process to go from 768 to 512 being only a projection, the change of dimensionality should not impact the results.
</Callout>

### Performance Requirements

<Cards>
  <Card title="Real-time Search" description="Search for nearest neighbors in real time with short search time" />
  <Card title="High Precision" description="Important to get logos as close as possible from the query logo for efficient annotation" />
  <Card title="Low Memory Usage" description="Must work in production on machines running other parallel tasks" />
</Cards>

## Test Conditions

<Callout type="info">
  All HNSW results were compared to FLAT faiss index search (an exact search) results to compute the precision of the index. Only **cosine similarity** was used in this benchmark.
</Callout>
...
<Callout type="warn">
  The recall never outreaches 0.76. A better recall was wanted, so Redis HNSW indexes were explored.
</Callout>
...
<Callout type="warn">
  **Memory Issue**: Redis loads all the embeddings in RAM when loading the index, which accounts for more than **20 GB of RAM** used by it. This is too much for our use case.
</Callout>
...
<Callout type="success">
  **Winner!** With the default parameters, Elasticsearch provided excellent results with optimal resource usage.
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    The RAM used is below **5 GB**, which meets our resource constraints.
  </Step>
...
<Callout type="success">
  **ElasticSearch was chosen** for computing the ANN search based on its superior performance, low memory usage, and acceptable search times.
</Callout>