...
<Step>
  **Upgrade from Git Repository** ```bash title="Upgrade Code" git fetch git
  checkout vX.y.z ```
</Step>

<Step>
  **Install Dependencies** ```bash title="Install Dependencies" poetry install
  ```
</Step>

<Step>
  **Database Migration** ```bash title="Run Database Migration" poetry run yoyo
