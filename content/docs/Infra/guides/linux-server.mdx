...
<Callout type="info">
  Every server is referenced in the [CT and VM list of OFF
  infrastructure](https://docs.google.com/document/d/14x5yPdcJ8uLoc5zb0HYjb_EYUIWZzOlaPWYyLSuIcTw/edit?resourcekey=0-h0h9ksbTUBykEDuBkXH4fg#).
</Callout>
...
<Cards>
  <Card title="Proxmox Mail Gateway" href="/docs/Infra/services/mail#servers">
...
<Tabs>
  <Tab value="ovh" label="OVH Servers">
    ### OVH Server Rules Rules are stored in: ```bash title="Rule Locations"
    /etc/iptables/rule.v4 # IPv4 rules /etc/iptables/rule.v6 # IPv6 rules ```
  </Tab>

  <Tab value="off1" label="OFF1 Server">
    ### OFF1 Server Rules Rules are stored in: ```bash title="Rule Location"
...
<Steps>
  ...
  <Step>
    **Reference Documentation** See [Docker iptables
    documentation](https://docs.docker.com/network/iptables/) for details.
  </Step>
</Steps>
...
<Steps>
  <Step>
    **Check Current Configuration** Compare current and default configurations:
    ```bash title="Compare Bashrc" diff $HOME/.bashrc /etc/skel/.bashrc ```
  </Step>

  <Step>
    **Copy Default Configuration** If different, copy the default: ```bash
...
<Tabs>
  <Tab value="color" label="No Color in Shell">
    ...
    <Steps>
      <Step>
        **Check TERM Variable** ```bash title="Check Terminal Type" echo $TERM
        ``` Should be `xterm-256color` or `linux`.
      </Step>

      <Step>
        **Fix Bashrc** ```bash title="Compare and Fix" diff $HOME/.bashrc
  ...
  <Tab value="completion" label="No Autocompletion">
    ...
    <Steps>
      <Step>
        **Check bash-completion Package** ```bash title="Verify Installation"
        dpkg --verify bash-completion && echo ok ```
      </Step>

      <Step>
        **Fix Bashrc Configuration** ```bash title="Compare and Fix" diff
