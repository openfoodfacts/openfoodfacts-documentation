...
<Steps>
  <Step>**Add Website** Go to manage / websites and add a website</Step>
</Steps>

### GDPR Compliance

<Callout type="info">To be GDPR compliant (and user friendly):</Callout>
...
<Callout type="info">
  **Performance Details:** - Systemd timers jobs process redis entries every
  minute (see `confs/matomo/systemd/matomo-tracking@.{(timer, service)}`) - We
  have instances 0, 1, 2,… 7, to process each queue - MariaDB configuration:
  `/etc/mysql/mariadb.conf.d/90-off-configs.cnf` (linked to this repository
  `confs/matomo/mysql/mariadb.conf.d/90-off-configs.cnf`) - We also tried to
  avoid "2006 MySQL server has gone away" following
