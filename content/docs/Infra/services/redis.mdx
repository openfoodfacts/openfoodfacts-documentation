...
<Steps>
  ...
  <Step>
    **Connection from OVH** To use from ovh1/ovh2, connect to stunnel client
    server on standard Redis port: ```bash title="Redis Connection" # Connect to
    Redis through stunnel redis-cli -h 10.1.0.113 -p 6379 ```
  </Step>
</Steps>
...
<Tabs>
  <Tab value="deployment" label="Deployment">
    ### Container Deployment - **Location**: Container 122 on the OFF Proxmox
    cluster - **Data Storage**: `/var/lib/redis` is in a specific dataset -
  ...
  <Tab value="configuration" label="Configuration">
    ...
    **System Service**: - Override systemd service to send emails on failure -
