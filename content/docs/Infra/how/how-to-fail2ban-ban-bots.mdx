...
<Steps>
  <Step>
    **Install Fail2ban** ```bash title="Install Fail2ban" sudo apt install
    fail2ban ```
  </Step>
  ...
  <Step>
    **Debian 11+ Configuration**
    <Callout type="info">
      ...
      fail2ban on debian 11+](/docs/Infra/how/how-to-fail2ban-debian-11+)
    </Callout>
  </Step>
</Steps>
...
<Steps>
  <Step>
    **Enable Manual Ban Jail** ```bash title="Enable nginx-manual-ban Jail" ln
    -s
    /opt/openfoodfacts-infrastructure/confs/common/fail2ban-nftables/jail.d/nginx-manual-ban.local
    /etc/fail2ban/jail.d/ systemctl restart fail2ban ```
  </Step>
