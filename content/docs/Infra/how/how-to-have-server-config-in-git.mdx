...
<Steps>
  ...
  <Step>
    **Clone Repository** ```bash title="Clone Infrastructure Repository" cd /opt
    git clone
    git@github.com-my-project:openfoodfacts/openfoodfacts-infrastructure.git ```
  </Step>
</Steps>
...
<Tabs>
  <Tab value="single" label="Single Repository">
    ...
    <Steps>
      <Step>
        **Copy Public Key** ```bash title="Get Public Key" cat
  ...
  <Tab value="multiple" label="Multiple Repositories">
    ...
    <Steps>
      <Step>
        **Create Project-Specific Key** ```bash title="Generate Project Key"
        ssh-keygen -t ed25519 -C "root@my-project-my-server-name" -f
        "/root/.ssh/github_my-project" cat /root/.ssh/github_my-project.pub ```
      </Step>

      <Step>
        **Configure SSH Client** Edit `/root/.ssh/config`: ```ini title="SSH
        Config" Host github.com-my-project Hostname github.com
        IdentityFile=/root/.ssh/github_my-project ```
      </Step>

      <Step>
        **Clone with Alias** ```bash title="Clone with SSH Alias" git clone
        git@github.com-my-project:my-org/my-project.git ```
      </Step>
    </Steps>
    <Callout type="info">
      See [GitHub
