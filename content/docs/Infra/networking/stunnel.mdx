...
<Tabs>
  <Tab value="client" label="Client Side">
    ### Stunnel Client - **Role**: Where connections are initiated (*Stunnel
    Server 1* in diagram) - **Configuration**: `client=yes` - **Security**: Must
    **never** be exposed on public IP - **Location**: Internal container with
    private IP only
  </Tab>

  <Tab value="server" label="Server Side">
    ### Stunnel Server - **Role**: Accepts connections and forwards to target
    service - **Configuration**: `client=no` (server side) - **Security**: May
    be exposed publicly through reverse proxy - **Location**: Reverse proxy
    configuration
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step>
    **Security Requirements** ```bash title="Secure PSK Files" chmod -R go-rwx
    /etc/stunnel/psk/ ```
    <Callout type="warning">
  ...
  <Step>**Generate PSK** ```bash title="Generate PSK" pwgen 32 ```</Step>
