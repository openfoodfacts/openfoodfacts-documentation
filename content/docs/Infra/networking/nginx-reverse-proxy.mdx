...
**Important**: Only the public IP should have a gateway. The default proxmox interface does not offer options to indicate which gateway should be the default gateway, and the public IP needs to have its gateway as the default one.
</Callout>
...
<Card title="Ban Bots" href="./how-to-fail2ban-ban-bots.md">
We ban bots from the reverse proxy (more efficient and centralized). Most of the time this is a manual ban.
</Card>
...
<Step>
**DNS Configuration**
In the DNS, CNAME your service name to:
...
```bash title="Test Wildcard Certificate"
...
See [Moji Datacenter](moji-datacenter.md) and [moji server installation report](reports/2024-08-13-moji-server-installation.md) for more information.
</Callout>