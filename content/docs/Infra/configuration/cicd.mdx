...
<Cards>
<Card title="Docker" href="./docker#technology-stack">
...
<Callout type="info">
The Makefile is the simplest entrypoint for collaborating to Open Food Facts repos, providing a uniform interface across different repositories.
</Callout>
...
<Callout type="info">
It is essential in the DevOps space, as it allows developers to frequently merge code changes into a central repository where builds and tests are run.
</Callout>
...
Run `style checks` as well as `unit`, `integration`, and `performance` tests on pull requests
</Card>
<Card title="Main Branch Deployment">
Deploy to a live environment and run integration tests on merge to `main` branch
</Card>
</Cards>
...
- On **push to tags matching `v*.*.*`**, run the `container-deploy.yml` workflow that will reload the container image to the off-org environment
</Tab>
...
**Important Notes:**
- Deployment process to pre-prod `.net` and production `.org` environments is identical (`container-deploy.yml`)
...
- Use GitHub SECRETS only for real secrets! For environment variables that depend on the deploy target, use [environment modification](https://docs.github.com/en/actions/using-workflows/workflow-commands-for-github-actions#setting-an-environment-variable) with an `if` directive
</Callout>
...
<Cards>
<Card title="Container deployment is failing">
...
<Card title="Forgot to set an env variable">
...
<Card title="Set up deployment on new repository">
...
<Card title="Deployment fails after merging PR">
...
<Card title="Release candidate before official release">
