...
<Tabs>
  <Tab value="logs" label="Log Collection">
    ### Log Management - **Filebeat**: Logs collection agent deployed on each
    QEMU VM with Docker containers -
    **[ElasticSearch](https://www.elastic.co/guide/en/elasticsearch/reference/current/index.html)**:
    Centralized storage and indexing of logs collected from Docker -
    **[Kibana](https://kibana.openfoodfacts.org/)**: UI to visualize and use
    logs collected by ElasticSearch [(official
    doc)](https://www.elastic.co/guide/en/kibana/current/index.html)
  </Tab>

  <Tab value="metrics" label="Metrics & Alerts">
    ### Metrics and Monitoring - **[Prometheus](https://prometheus.io/docs)**:
    Scrapes metrics from Prometheus exporters' `/metrics` endpoint -
    **AlertManager**: Sends alerts based on Prometheus metrics, integrated with
    dedicated Slack channels -
    **[InfluxDB](https://docs.influxdata.com/influxdb/v2.0/)**: Storage backend
    for data harvested by Prometheus -
    **[Grafana](https://grafana.openfoodfacts.org/)**: Visualizing Prometheus
    metrics, InfluxDB and other metrics; creating dashboards [(official
    doc)](https://grafana.com/docs/grafana/latest)
  </Tab>

  <Tab value="exporters" label="Exporters">
    ### Prometheus Exporters - **Prometheus exporters**: Such as the [Apache
    ...
    Prometheus metric format - Some applications natively export Prometheus
...
<Steps>
  ...
  <Step>
    **Parameter Handling** - Simple params like `instance`, `app` are parameters
    for the job configuration, not for the target URL - To add params to target
    URL we use `__param_xxxx` (for example `__param_target` to add a `target`
    parameter to URL) - `instance` is very important, as each job must have
    their separate instance name
  </Step>
</Steps>
