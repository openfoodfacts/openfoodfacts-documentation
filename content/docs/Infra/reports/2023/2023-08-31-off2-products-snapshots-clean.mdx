---
title: "OFF2 Products Snapshots Clean"
description: "This report details the cleanup of ZFS snapshots on off2's `zfs-nvme` pool to resolve disk space issues. It covers modifying the `snapshot_purge` script, manually purging old snapshots for opf and obf, and disabling `atime` for performance."
...
I forgot to change that when I moved the dataset to zfs-nvme during [off install on off2](/docs/Infra/reports/2023/2023-07-off2-off-reinstall).
