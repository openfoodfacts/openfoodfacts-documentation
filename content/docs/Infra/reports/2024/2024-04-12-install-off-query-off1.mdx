---
title: "Install Off-Query on Off1"
description: "This report details the installation of off-query on an unprivileged LXC container on off1. It covers setting up the container, installing Docker, configuring SSH, deploying off-query, setting up the reverse proxy, and migrating data."
...
I setup the reverse proxy on off2 to [configure a new service](/docs/Infra/services/nginx-reverse-proxy#configuring-a-new-service)
...
[we can't access services hosted on same proxmox cluster using the off2 reverse proxy](/docs/Infra/services/nginx-reverse-proxy#never-call-an-internal-service-using-reverse-proxy).
