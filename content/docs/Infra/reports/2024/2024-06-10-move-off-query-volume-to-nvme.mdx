---
title: "Move Query Volume"
description: "This report details moving the off-query Docker volume from ZFS-HDD to NVME on off1. It covers the process of adding a new mount point, copying data, troubleshooting unexpected reboots, and switching volumes to improve performance and reliability for the off-query service."
...
```bash
curl -d '[{"$match": {"countries_tags": "en:north-korea"}},{"$group":{"_id":"$brands_tags"}}]' -H "Content-Type: application/json" http://10.1.0.103:16001/select
