...
There is a caveat if you [use the .envrc file](./how-to-use-direnv) (which as the advantage to be ignored by git), because, in `.env` we define the separator between the different `COMPOSE_FILE` to be `;` which does not work with the `.envrc` syntax.
