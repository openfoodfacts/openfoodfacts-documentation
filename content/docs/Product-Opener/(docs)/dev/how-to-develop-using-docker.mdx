---
title: "How to use Docker to Develop - a guide"
---
...
```bash title="Tail Docker Compose logs"
make log
```
...
```bash title="Increasing log verbosity"
...
```bash title="Opening a shell in a Docker container"
...
```bash title="Checking permissions"
ls -lrt
```
...
```bash title="Running minion jobs"
...
```bash title="Run queries on MongoDB database"
...
```mermaid title="Adding environment variables"
stateDiagram-v2
  make_up: make up
