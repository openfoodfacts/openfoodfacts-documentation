---
title: "Quick Start"
description: "Complete guide to quickly set up a development environment for Open Food Facts Product Opener using Docker with step-by-step instructions."
---
...
<Card title="Alternative: Cloud Development" href="/docs/Product-Opener/dev/development-environment/how-to-use-gitpod">
  Follow the Gitpod how-to guide to instantly provision a ready-to-code
...
- `8 GB` RAM dedicated to Docker client
...
Check [how to learn perl](/docs/Product-Opener/dev/reference/how-to-learn-perl)!
...
<Cards>
  <Card title="Git Version" description="≥ 2.25.0" />
  <Card
    title="Docker CE"
...
On macOS, the default version of **grep** is BSD grep, which does not support certain GNU-specific options—most notably, the `-P` flag for Perl-compatible regular expressions.

</Callout>

If you encounter errors like:

```console title="BSD grep Error"
...
```bash title="Verify GNU grep"
grep --version
```

You should see output similar to this, indicating GNU grep with PCRE2 support:
...
>
  When running with Windows, install Docker Desktop which will cover all of the
  above.
</Card>
...
The Make tasks use a number of Linux commands, such as rm and nproc, so it is recommended to run Make commands from the Git Bash shell.
...
<Cards>
  ...
  <Card
    ...
    description="For downloading the full product database"
  />
</Cards>

<Callout>
**wget Installation Tip**

If you want to download wget with the executable, copy the wget.exe file to C:/Windows/System32 and you are done.

</Callout>

#### Symbolic Links Permission Setup

<Callout type="warn">
**Windows Symbolic Links**

The process of cloning the repository will create a number of symbolic links which require specific permissions under Windows.

</Callout>

<Tabs items={["Administrative Access", "UAC Disable", "Specific Permission"]}>
  <Tab value="Administrative Access">
    Use an Administrative command prompt for all Git commands
  </Tab>
  <Tab value="UAC Disable">Completely disable UAC</Tab>
  <Tab value="Specific Permission">
    Specifically grant the [Create symbolic
    links](https://learn.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/create-symbolic-links)
...
If you want to collaborate you will need to fork the repository. (See [*Creating a pull request from a fork* in Github documentation](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request-from-a-fork) for an explanation.)
...
<Tabs items={['On Windows', 'On other systems']}>
<Tab value="On Windows">
...
**Note:**
Instead of modifying `.env` (and risk committing it inadvertently), you can also set needed variables in your shell; they will override `.env` values. Consider creating a `.envrc` file that you source each time you need to work on the project. On linux and macOS, you can automatically do it if you [use direnv](/docs/Product-Opener/dev/development-environment/how-to-use-direnv).
...
{/* Note: If you are using Windows, you may encounter issues regarding this command. Take a look at the Troubleshooting section further in this tutorial. */}

{/* Note: If docker complains about ERROR: could not find an available, non-overlapping IPv4 address pool among the defaults to assign to the network, It can be solved by adding {"base": "172.80.0.0/16","size": 24}, {"base": "172.90.0.0/16","size": 24} to default-address-pools in /etc/docker/daemon.json and then restarting the docker daemon. */}
...
- For a full description of available make targets, see [Docker / Makefile commands](/docs/Product-Opener/dev/reference/ref-docker-commands)
...
To learn more about developing with Docker, see the [Docker developer's guide](/docs/Product-Opener/dev/development-environment/how-to-develop-using-docker).

To have all site pages on your dev instance, see [Using pages from openfoodfacts-web](/docs/Product-Opener/dev/development-guides/how-to-use-pages-from-openfoodfacts-web)

[Using Repl](/docs/Product-Opener/dev/development-environment/how-to-use-repl) offers you a way to play with perl.

Specific notes are provided on [applying AGRIBALYSE updates to support the Ecoscore](/docs/Product-Opener/dev/development-guides/how-to-update-agribalyse-ecoscore) calculation.
...
Click the Windows button, then type "environment properties" into the search bar and hit Enter. Click Environment Variables, then under System variables choose Path and click Edit. Click New and insert C:\Program Files (x86)\GnuWin32\bin, then save the changes. Open a new terminal and test that the command works. (See [Make Windows](https://pakstech.com/blog/make-windows/) for more.)
...
### make dev error: [build_lang] Error 2 - Could not load taxonomy
...
**Solution:**
Project needs Symlinks to be enabled. traces.result.sto is a symlink to allergens.result.sto
...
On macOS, when running `make dev`:
...
### make dev error: [build_lang] Error 13 - can't write into directory

When running `make dev`:

```console
can't write into /mnt/podata/data/Lang.openfoodfacts.localhost.sto: Permission denied at /opt/product-opener/lib/ProductOpener/Store.pm line 234.
```

**Solution:**
Use the powershell/cmd to run the make dev commands in windows.

### make dev error: [build] Error 1 - failed to solve
...
Reason for the error: "usermod: UID '0' already exists" means that the user is being assigned UID 0, which is already used by the root user.
...
### make dev error: permission denied while trying to connect to Docker daemon
