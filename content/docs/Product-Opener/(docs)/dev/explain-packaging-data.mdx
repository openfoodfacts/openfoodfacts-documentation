...
description: "Comprehensive explanation of how Open Food Facts handles packaging data including structure, input fields, taxonomies, and improvement strategies."
---
...
<Steps>
<Step title="Packaging tag field (READ and WRITE)">
...
<Step title="Packaging information / recycling instructions text field (READ and WRITE)">
...
<Steps>
<Step title="Extract attributes that relate to different packaging components">
The values for each input field ("packaging" tag field and "packaging_text_[language code]" packaging information text field) are analyzed{/* parse_packaging_from_text_phrase() function in /lib/ProductOpener/Packagings.pm */} to recognize packaging components and their attributes. One product may have multiple "packaging_text_[language code]" values in different languages. Only the value for the main language ("lang" field) of the product is currently analyzed.
...
```json title="Merged Packaging Data Structure Example"
{
  "packagings": [
    {
      "material": "en:pet-polyethylene-terephthalate",
...
- **Packaging shapes taxonomy**: [packaging_shapes.txt](https://github.com/openfoodfacts/openfoodfacts-server/blob/main/taxonomies/packaging_shapes.txt)
- **Packaging materials taxonomy**: [packaging_materials.txt](https://github.com/openfoodfacts/openfoodfacts-server/blob/main/taxonomies/packaging_materials.txt)
- **Packaging recycling taxonomy**: [packaging_recycling.txt](https://github.com/openfoodfacts/openfoodfacts-server/blob/main/taxonomies/packaging_recycling.txt)
- **Preservation methods taxonomy** (related): [preservation.txt](https://github.com/openfoodfacts/openfoodfacts-server/blob/main/taxonomies/preservation.txt)
...
<Steps>
<Step title="Extend the attributes of the packaging components in the packagings data structure">
  #### Weight
...
<Step title="Make the packagings data structure READ and WRITE">
...
<Step title="Add a way to indicate that the packagings data structure contains all the packaging components of the product">
...
<Step title="Deprecate the packaging tags field">
...
<Step title="Keep the packaging_text_[language code] field">
...
- 2020 project to start structuring packaging data: [Packagings data structure wiki](https://wiki.openfoodfacts.org/Packagings_data_structure)
