...
<Steps>
<Step>
**Handle Missing Data**: If you can't get the information on a specific product, get your user to send photos and data
</Step>
<Step>
**Process Data**: Data will be processed by Open Food Facts to get the computed result
</Step>
<Step>
**Immediate Results**: Implement the complete flow so users get immediate results with minimal effort
</Step>
</Steps>

<Card title="Technical Implementation" href="./adding-missing-products.mdx">
Refer to the product addition tutorial for technical operations such as category input
</Card>

## Table of contents

<Cards>
<Card title="App Preparation" description="Getting your app ready for the Green-Score" />
<Card title="Basic Display" description="Implementing the basic display of the score" />
<Card title="International Support" description="Displaying the Green-Score outside France" />
<Card title="User Experience" description="Ensuring good UX even with data gaps" />
...
<Callout type="info" title="Handling Missing Green-Score (Category)">
  If `"en:categories-completed"` is in `states_tags` AND `ecoscore_grade` is `Null`:
  We could not compute a Green-Score for this product. It might be that the category is not specific enough or that we don't have supporting data for this category. If you believe this is an error, you can email [contact@example.com](mailto:contact@example.com).
...
<Callout type="info" title="Helping Users Add Missing Categories">
  You can use our Robotoff API to get your users to validate a prediction. Refer to [Robotoff Questions](https://docs.google.com/document/d/1IoDy0toQrrqtWHvDYp2rEVw84Yq1J0x2pt-0RGTm7h0/edit).
</Callout>

<Callout type="info" title="Handling Missing Labels">
  If `"en:labels-to-be-completed"` is in `states_tags`:
  ...
  Asking your users for a photo should be enough. You can otherwise add toggles for Explicit labels (please add a photo of them to avoid mistakes).
  You can get states with [https://world.openfoodfacts.org/api/v0/product/3414280980209.json?fields=environmental_score_grade,states_tags](https://world.openfoodfacts.org/api/v0/product/3414280980209.json?fields=environmental_score_grade,states_tags)
</Callout>

<Callout type="info" title="Handling Missing Origins">
  If `"en:origins-to-be-completed"` is in `states_tags`:
...
<Callout type="info" title="Handling Missing Recycling Information">
  If `"en:packaging-photo-to-be-selected"` is in `states_tags`:
  ...
  If `"en:packaging-to-be-completed"` is in `states_tags`:
  ...
  The field to input raw recycling instructions e.g., "Plastic bottle to recycle, Plastic cap to recycle" is "packaging_text_en" (change the language code accordingly). It will get automatically parsed and get used to compute the Green-Score.
