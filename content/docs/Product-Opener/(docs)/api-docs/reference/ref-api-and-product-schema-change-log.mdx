...
description: "Complete reference documenting changes to the Open Food Facts API and product schema versions with breaking and non-breaking modifications."
---
...
<Step title="Breaking vs. Non-Breaking Changes">
...
<Step title="Undocumented Fields">
...
- the "images" structure has been changed to better separate uploaded and selected images
...
- The brands field is now taxonomized. brands_tags entries are prefixed with the language-less xx: prefix, and there is a new brands_hierarchy field ([see tags schema](/ref/schemas/product_tags.yaml)) - [PR](https://github.com/openfoodfacts/openfoodfacts-server/pull/11606)

Non-breaking changes:

- Added schema_version field

### 2024-12-12 - Product version 1000 - API version 3.1 - Renamed ecoscore*\* fields to environmental_score*\*

Breaking changes:

- For legal reasons, we had to rename the Eco-Score to Green-Score. To make sure we won't have to update the schema again, we renamed the corresponding fields from ecoscore*\* to the generic name environmental_score*\*
...
The normalization of leading 0s has been changed. See [Barcode normalization](/docs/Product-Opener/api-docs/reference/ref-barcode-normalization)
