---
title: "API Tutorial"
description: "Complete tutorial on using Open Food Facts API to get Nutri-scores, complete product data, and search products."
---
...
<Callout type="info">
  First, be sure to see our [Introduction to the API](/docs/Product-Opener/api-docs/index).
</Callout>
...
To get a product nutriscore, send a request to the [Get A Product By Barcode](/docs/Product-Opener/v2#get-/api/v2/product/-barcode-) endpoint.

<Steps>
<Step title="Authentication">
Usually, no authentication is required to query Get A Product Nutri-score. However, there is a basic auth to avoid content indexation in the staging environment (which is used throughout this tutorial). For more details, visit the [Open Food Facts API Environment](/docs/Product-Opener/api-docs/index#api-deployments).
</Step>
<Step title="Describing the Get Request">
...
<Step title="Nutri-Score Computation">
...
```bash title="Request with Additional Fields"
...
For more details, see the reference documentation for [Get A Product By Barcode](/docs/Product-Opener/v2#get-/api/v2/product/-barcode-).
...
<Steps>
<Step title="Products without a Nutri-Score">
...
```json title="Missing Tags Response"
{
  "code": "0180411000803",
  "product": {
    "misc_tags": [
...
<Step title="Write data to make Nutri-Score computation possible">
...
To write data to a product, make a `POST` request to the [`Add or Edit A Product`](/docs/Product-Opener/v2#post-/cgi/product_jqm2.pl) endpoint.
...
```json title="Fields Saved Response"
{
...
<Step title="Read newly computed Nutri-Score">
...
```json title="Computed Nutri-Score Response"
{
    "code": "0180411000803",
    "product": {
        "nutriments": {
...
For more details, see the reference documentation for [Add or Edit A Product](/docs/Product-Opener/v2#post-/cgi/product_jqm2.pl)
...
Using the Open Food Facts API, you can filter products based on different criteria. To search for products in the Orange Juice category with a nutrition_grade of `c`, query the [Search for Products endpoint](/docs/Product-Opener/v2#get-/api/v2/search).
...
```json title="Product Search Response"
{
  "count": 1629,
  "page": 1,
  "page_count": 24,
...
```json title="Sorted Product Search Response"
{
  "count": 1629,
  "page": 1,
  "page_count": 24,
...
To see other examples of sorting a search response, see the reference documentation for [Search for Products](/docs/Product-Opener/v2#get-/api/v2/search).
