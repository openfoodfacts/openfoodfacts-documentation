...
<Steps>
  ...
  <Step>
    **Follow the technical guide** in the [product addition
    tutorial](../tutorials/adding-missing-products) for required operations
  </Step>
</Steps>
...
<Card title="Robotoff API Integration" href="https://docs.google.com/document/d/1IoDy0toQrrqtWHvDYp2rEVw84Yq1J0x2pt-0RGTm7h0/edit">
...
<Card title="Image Refresh API" href="https://github.com/openfoodfacts/api-documentation/issues/15">
Learn more about refreshing obsolete nutrition images
</Card>
</Step>
</Steps>
...
<Steps>
  <Step title="A message if fibers are missing">
    ```json title="Fiber Warning Message" msgctxt
    "nutrition_grade_fr_fiber_warning" msgid "Warning: the amount of fiber is
    not specified, their possible positive contribution to the grade could not
    be taken into account." ```
  </Step>
  <Step title="A message if fruit/nuts are missing">
    ```json title="Fruits/Vegetables/Nuts Warning" msgctxt
    "nutrition_grade_fr_no_fruits_vegetables_nuts_warning" msgid "Warning: the
    amount of fruits, vegetables and nuts is not specified, their possible
    positive contribution to the grade could not be taken into account." ```
  </Step>
  <Step title="A message if fruits/nuts is an estimate from ingredients">
    ```json title="Ingredients Estimate Warning" msgctxt
    "nutrition_grade_fr_fruits_vegetables_nuts_estimate_warning" msgid "Warning:
    the amount of fruits, vegetables and nuts is not specified on the label, it
    was estimated from the list of ingredients: %d%" ```
  </Step>
  <Step title="A message if fruits/nuts is an estimate from category">
    ```json title="Category Estimate Warning" msgctxt
    "nutrition_grade_fr_fruits_vegetables_nuts_from_category_warning" msgid
