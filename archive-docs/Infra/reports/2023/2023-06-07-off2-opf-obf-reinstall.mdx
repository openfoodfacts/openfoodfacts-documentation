---
title: OPF OBF Reinstall
description: This document details the reinstallation process for Open Products Facts and Open Beauty Facts on OFF2, covering ZFS dataset setup, data migration, container creation, package installation, volume mounting, and service configuration. It also includes troubleshooting steps and annexes for broken links and file differences.
...
They already are synced (cf. [opff reinstall on off2](/docs/Infra/reports/2023/2023-03-14-off2-opff-reinstall#products-for-all-flavors))
...
I created a CT for obf followings [How to create a new Container](/docs/Infra/virtualization/proxmox#how-to-create-a-new-container) it went all smooth.
...
**Warning**: after changing lxc idmap, the alex user which I already created, now as an id which is not mapped correctly. More over it was a mistake to create it immediatly as it have id 1000, which should be reserved to _off_ user. This makes it impossible to log with ssh as .ssh/authorized_keys is not readable !
...
5. shutdown obf on both side
   on off1: `sudo systemctl stop apache2@obf` and `unlink /etc/nginx/sites-enabled/obf && systemctl reload nginx`

6. change DNS to point to new machine

7. Rsync and zfs sync again

   rsync took 8 minutes

8. ensure migrations works using NFS for off1 apps:
...
9. ensure migrations works using mounts for off2 apps:
...
10. start obf on container off2/111 (obf): `sudo systemctl start apache2 nginx`
11. check it works (remember to also clean your /etc/hosts if you modified it for tests)
12. disable obf service on off1:
    - `systemctl disable apache2@obf`
    - `unlink /etc/nginx/sites-enabled/obf && sytemctl reload nginx` (if not already done)

13. remove obf from snapshot-purge.sh on ovh3 (now handled by sanoid)
14. on off2 and ovh3 modify sanoid configuration to have obf/products handled by sanoid and synced to ovh3
15. don't forget to test that it still works after that
...
10. start opf on container off2/112 (opf): `sudo systemctl start apache2 nginx`
11. check it works (remember to also clean your /etc/hosts if you modified it for tests)
12. disable opf service on off1:
    - `systemctl disable apache2@opf`
    - `unlink /etc/nginx/sites-enabled/opf && sytemctl reload nginx` (if not already done)

13. remove opf from snapshot-purge.sh on ovh3 (now handled by sanoid)
14. on off2 and ovh3 modify sanoid configuration to have opf/products handled by sanoid and synced to ovh3
15. don't forget to test that it still works after that
