---
title: "Kimsufi KS1 Installation"
description: "This report details the installation and configuration of a new Kimsufi KS1 server to offload image serving from existing servers. It covers server specifications, Debian 12 installation, ZFS setup, data synchronization, firewall configuration, and Nginx setup with SSL certificates."
...
I also followed [How to have server config in git](/docs/Infra/how/how-to-have-server-config-in-git)
...
```
and following [Server, postfix configuration](/docs/Infra/services/mail#postfix-configuration).

I also had to had ks1 ip address to [forwarding rules on ovh1 to the mail gateway](/docs/Infra/services/mail#redirects).
```bash
...
So I created the pool with them (see [How to create a zpool](/docs/Infra/storage/zfs-overview#how-to-create-a-zpool))
...
I created a ks1operator user on ovh3, following [creating operator on PROD_SERVER](/docs/Infra/storage/sanoid#creating-operator-on-prod_server)
