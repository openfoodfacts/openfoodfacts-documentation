---
title: "Debug Minion Tasks"
description: "Comprehensive guide to debugging Minion import and export tasks including log analysis, job status checking, and troubleshooting failed tasks."
---
...
```bash title="Minion Job Status Example"
...
### Getting debug level logs

To get more data (debug level) in `minion_log4perl.log`:

<Steps>
<Step title="Stop Minion Daemon">
**Stop the minion daemon** (as root user):
```bash
systemctl stop minion@off.service
```
</Step>
<Step title="Run Manually">
**Run it manually** (as off user):
```bash
sudo -u off bash
